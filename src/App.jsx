...
function StorefrontMini({data,products,editor,full=false,onAdd,cartCount=0}) {
  ...
  return <div className={`storefront ${full?'full-storefront':''}`} style={{'--brand':data.primaryColor,'--paper':data.secondaryColor,'--accent':data.accentColor,'--section-gap':`${theme.sectionGap||32}px`,'--card-radius':`${theme.radius||0}px`}}><header><div className="store-logo">{data.businessName||'Your Store'}</div><nav><span>Shop</span><span>About</span><span>Journal</span></nav><div><Search size={15}/><span className="store-cart-indicator"><ShoppingBag size={16}/>{full&&cartCount>0&&<b>{cartCount}</b>}</span></div></header><section className={`sf-hero align-${editor.hero.align}`}><div><small>{editor.hero.eyebrow}</small><h1>{editor.hero.heading}</h1><p>{editor.hero.body}</p><button>{editor.hero.button}</button></div><div className="sf-hero-art"><div className="art-object"><i/><b/></div></div></section><section className="sf-products"><div className="sf-section-head"><h2>{editor.featured.title}</h2><span>{visible.length} products</span></div><div className={`sf-product-grid columns-${editor.featured.columns}`}>{visible.map((p,i)=><article key={p.id}><div className={`sf-product-image product-art-${(i%4)+1}`}><div/></div><h3>{p.name}</h3><p>{formatPrice(p.price)}</p>{full&&<button className="sf-add-cart" onClick={()=>onAdd?.(p)}>Add to cart</button>}</article>)}</div>{!visible.length&&<div className="empty-panel"><Package size={22}/><strong>No active products</strong><p>Activate a product in Catalog to show it in the storefront.</p></div>}</section><section className="sf-story"><small>OUR APPROACH</small><h2>{editor.story.title}</h2><p>{editor.story.body}</p></section>{blocks.map(block=><section key={block.id} className="sf-custom-block" style={{background:block.background||'#fff',color:block.text||'#171717',padding:`${block.padding||48}px`,borderRadius:`var(--card-radius)`}}><div className="sf-custom-inner" style={{display:'grid',gridTemplateColumns:`repeat(${block.columns||1}, minmax(0, 1fr))`,gap:'20px'}}>{Array.from({length:block.columns||1}).map((_,idx)=><div key={idx}>{block.type==='image'&&block.imageUrl?<img src={block.imageUrl} alt={block.title||'Content image'}/>:null}<h2>{block.title}</h2>{block.type==='text'&&<p>{block.body}</p>}{['list','menu'].includes(block.type)&&<ul>{String(block.items||'').split(',').map(x=>x.trim()).filter(Boolean).map(item=><li key={item}>{item}</li>)}</ul>}{block.type==='image'&&block.body&&<p>{block.body}</p>}</div>)}</div></section>)}<section className="sf-newsletter"><h2>Notes from the studio.</h2><p>Occasional stories, new arrivals, and thoughtful things worth sharing.</p><div><span>Email address</span><button>Join</button></div></section><footer><strong>{data.businessName||'Your Store'}</strong><span>Shop · About · Contact · Instagram</span><small>© 2026 {data.businessName||'Your Store'}</small></footer></div>
}

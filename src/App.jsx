...
function AppShell({ page, setPage, children, onRestart, businessName }) {
  ...
  return <div className="app-shell">
    <aside className={`app-sidebar ${mobile?'open':''}`}>
      ...
      <div className="sidebar-bottom"><button><Settings size={18}/> Settings</button><button><CircleHelp size={18}/> Help</button><button onClick={onRestart}><Sparkles size={18}/> Store setup</button></div>
    </aside>
...
function Dashboard({ data, products, setPage }) {
  ...
  return <div className="page-wrap">
    ...
    <div className="stat-grid"><Stat title="Sales" value="₱0" note="No sales yet" icon={BarChart3}/><Stat title="Orders" value="0" note="No orders yet" icon={ShoppingBag}/><Stat title="Conversion" value="—" note="Available after traffic" icon={Store}/><Stat title="Customers" value="0" note="No customers yet" icon={Users}/></div>
...
function Products({products,setProducts}) {
  ...
  const [draft,setDraft]=useState({name:'',price:'',inventory:'',category:''})
  const add=()=>{ if(!draft.name) return; setProducts(prev=>[...prev,{id:Date.now(),name:draft.name,price:Number(draft.price||0),inventory:Number(draft.inventory||0),category:draft.category||'Uncategorized',status:'Draft'}]); setDraft({name:'',price:'',inventory:'',category:''}); setAdding(false) }
  return <div className="page-wrap"><div className="page-head"><div><p className="overline">CATALOG</p><h1>Products</h1><p>Create the items that will appear in the storefront.</p></div><Button onClick={()=>setAdding(true)}><Plus size={16}/> Add product</Button></div><div className="toolbar"><div className="searchbox"><Search size={16}/><input placeholder="Search products"/></div><Button variant="secondary">All products <ChevronDown size={14}/></Button></div><div className="table-card"><table><thead><tr><th>Product</th><th>Status</th><th>Inventory</th><th>Category</th><th>Price</th></tr></thead><tbody>{products.map((p,i)=><tr key={p.id}><td><div className={`product-thumb thumb-${(i%4)+1}`}><ShoppingBag size={18}/></div><strong>{p.name}</strong></td><td><span className={`status ${p.status.toLowerCase()}`}>{p.status}</span></td><td>{p.inventory} in stock</td><td>{p.category}</td><td>{formatPrice(p.price)}</td></tr>)}</tbody></table></div>{adding&&<Modal title="Add product" onClose={()=>setAdding(false)}><div className="modal-form"><Field label="Product name"><input value={draft.name} onChange={e=>setDraft({...draft,name:e.target.value})} placeholder="Product name"/></Field><div className="form-grid two"><Field label="Price"><input type="number" value={draft.price} onChange={e=>setDraft({...draft,price:e.target.value})} placeholder="0"/></Field><Field label="Inventory"><input type="number" value={draft.inventory} onChange={e=>setDraft({...draft,inventory:e.target.value})} placeholder="0"/></Field></div><Field label="Category"><input value={draft.category} onChange={e=>setDraft({...draft,category:e.target.value})} placeholder="e.g. Lighting"/></Field><div className="upload-zone"><Upload size={22}/><strong>Product images</strong><span>Add product media from the Media library after creating the item.</span></div><div className="modal-actions"><Button variant="secondary" onClick={()=>setAdding(false)}>Cancel</Button><Button onClick={add}>Create draft</Button></div></div></Modal>}</div>
}
...
function OperationsPage({type}) {
  if(type==='orders') return <div className="page-wrap"><div className="page-head"><div><p className="overline">COMMERCE</p><h1>Orders</h1><p>Track purchases, payment status, and fulfillment from one place.</p></div><Button variant="secondary">Export</Button></div><div className="stat-grid compact-stats"><Stat title="Orders today" value="0" note="No orders yet" icon={ShoppingBag}/><Stat title="Open orders" value="0" note="Nothing to fulfill" icon={Package}/><Stat title="Revenue" value="₱0" note="Today" icon={BarChart3}/><Stat title="Average order" value="—" note="After your first sale" icon={BriefcaseBusiness}/></div><EmptyState icon={ShoppingBag} title="Your orders will appear here" body="When customers complete checkout, CoBest will organize payment and fulfillment details in this workspace."/></div>
  if(type==='customers') return <div className="page-wrap"><div className="page-head"><div><p className="overline">COMMERCE</p><h1>Customers</h1><p>Keep customer details and purchase history connected to the store.</p></div><Button><Plus size={16}/> Add customer</Button></div><EmptyState icon={Users} title="No customers yet" body="Customer profiles will appear here as people purchase or are added to the store." action="Add customer"/></div>
  if(type==='media') return <div className="page-wrap"><div className="page-head"><div><p className="overline">CONTENT</p><h1>Media</h1><p>Keep website and product assets organized in one library.</p></div><Button><Upload size={16}/> Add media</Button></div><EmptyState icon={ImageIcon} title="Build your media library" body="Upload your logo, product photography, lifestyle images, and website assets so they are ready everywhere you need them." action="Add media"/></div>
  if(type==='analytics') return <div className="page-wrap"><div className="page-head"><div><p className="overline">ANALYTICS</p><h1>Store performance</h1><p>Review sales and storefront activity from one business view.</p></div><Button variant="secondary">Last 30 days <ChevronDown size={14}/></Button></div><div className="stat-grid"><Stat title="Sales" value="₱0" note="Last 30 days" icon={BarChart3}/><Stat title="Orders" value="0" note="Completed and open" icon={ShoppingBag}/><Stat title="Conversion" value="—" note="Available after traffic" icon={Store}/><Stat title="Customers" value="0" note="Returning + new" icon={Users}/></div><EmptyState icon={BarChart3} title="Analytics start with real traffic" body="Once your storefront is live and receiving visitors, performance data will appear here."/></div>
  if(type==='marketing') return <div className="page-wrap"><div className="page-head"><div><p className="overline">GROWTH</p><h1>Marketing</h1><p>Organize store campaigns and customer touchpoints around the same business.</p></div><Button><Plus size={16}/> Create campaign</Button></div><EmptyState icon={Sparkles} title="Create your first campaign" body="Plan email, launch, and promotional activity around the same products and storefront content." action="Create campaign"/></div>
  if(type==='discounts') return <div className="page-wrap"><div className="page-head"><div><p className="overline">COMMERCE</p><h1>Discounts</h1><p>Create offers that can be used across the storefront.</p></div><Button><Plus size={16}/> Create discount</Button></div><EmptyState icon={Sparkles} title="No discounts yet" body="Create discount codes and promotional offers when you are ready to run a campaign." action="Create discount"/></div>
  return null
}

function Editor({data,products,editor,setEditor}) {
  const sections=[['header','Header'],['hero','Hero'],['featured','Featured products'],['story','Brand story'],['newsletter','Newsletter'],['footer','Footer']]
  const update=(section,key,value)=>setEditor(prev=>({...prev,[section]:{...prev[section],[key]:value}}))
  return <div className="editor-screen"><div className="editor-top"><div><Logo/><span className="editor-divider"/><strong>Home</strong><ChevronDown size={14}/></div><div className="device-toggle">{[['desktop',Monitor],['tablet',Tablet],['mobile',Smartphone]].map(([id,I])=><button key={id} className={editor.device===id?'active':''} onClick={()=>setEditor({...editor,device:id})}><I size={16}/></button>)}</div><div><Button variant="ghost"><Eye size={15}/> Preview</Button><Button>Save</Button></div></div><div className="editor-body"><aside className="section-panel"><div className="panel-title"><span>Page sections</span><button><Plus size={16}/></button></div>{sections.map(([id,label],i)=><button key={id} className={`section-item ${editor.selected===id?'active':''}`} onClick={()=>setEditor({...editor,selected:id})}><span className="drag-dots">⠿</span><div className="section-thumb">{i<2?<ImageIcon size={14}/>:<LayoutDashboard size={14}/>}</div><strong>{label}</strong></button>)}<button className="add-section"><Plus size={15}/> Add section</button></aside><main className="canvas-area"><div className={`store-canvas device-${editor.device}`}><StorefrontMini data={data} products={products} editor={editor}/></div></main><aside className="settings-panel"><div className="settings-head"><span>Section settings</span><strong>{sections.find(s=>s[0]===editor.selected)?.[1]}</strong></div>{editor.selected==='hero'&&<div className="settings-form"><Field label="Eyebrow"><input value={editor.hero.eyebrow} onChange={e=>update('hero','eyebrow',e.target.value)}/></Field><Field label="Heading"><textarea rows="3" value={editor.hero.heading} onChange={e=>update('hero','heading',e.target.value)}/></Field><Field label="Body"><textarea rows="4" value={editor.hero.body} onChange={e=>update('hero','body',e.target.value)}/></Field><Field label="Button label"><input value={editor.hero.button} onChange={e=>update('hero','button',e.target.value)}/></Field><Field label="Alignment"><div className="segment"><button className={editor.hero.align==='left'?'active':''} onClick={()=>update('hero','align','left')}>Left</button><button className={editor.hero.align==='center'?'active':''} onClick={()=>update('hero','align','center')}>Center</button></div></Field></div>}{editor.selected==='featured'&&<div className="settings-form"><Field label="Section heading"><input value={editor.featured.title} onChange={e=>update('featured','title',e.target.value)}/></Field><Field label="Columns"><div className="segment">{[2,3,4].map(n=><button key={n} className={editor.featured.columns===n?'active':''} onClick={()=>update('featured','columns',n)}>{n}</button>)}</div></Field></div>}{editor.selected==='story'&&<div className="settings-form"><Field label="Heading"><input value={editor.story.title} onChange={e=>update('story','title',e.target.value)}/></Field><Field label="Body"><textarea rows="5" value={editor.story.body} onChange={e=>update('story','body',e.target.value)}/></Field></div>}{!['hero','featured','story'].includes(editor.selected)&&<div className="empty-settings"><Settings size={24}/><strong>Basic section</strong><p>Use the section controls to manage its content, layout, and visibility.</p></div>}<div className="project-context"><span>PROJECT CONTEXT</span><strong>{data.goals[0]||'Website goal'}</strong><p>{data.primaryAction}</p><div><b>Direction</b><em>{data.styles.join(' + ')}</em></div><div><b>Audience</b><em>{data.audience}</em></div></div></aside></div></div>
}

function StorefrontMini({data,products,editor,full=false}) {
  const visible = products.filter(x=>x.status==='Active')
  return <div className={`storefront ${full?'full-storefront':''}`} style={{'--brand':data.primaryColor,'--paper':data.secondaryColor,'--accent':data.accentColor}}><header><div className="store-logo">{data.businessName}</div><nav><span>Shop</span><span>About</span><span>Journal</span></nav><div><Search size={15}/><ShoppingBag size={16}/></div></header><section className={`sf-hero align-${editor.hero.align}`}><div><small>{editor.hero.eyebrow}</small><h1>{editor.hero.heading}</h1><p>{editor.hero.body}</p><button>{editor.hero.button}</button></div><div className="sf-hero-art"><div className="art-object"><i/><b/></div></div></section><section className="sf-products"><div className="sf-section-head"><h2>{editor.featured.title}</h2><span>View all →</span></div><div className={`sf-product-grid columns-${editor.featured.columns}`}>{visible.map((p,i)=><article key={p.id}><div className={`sf-product-image product-art-${(i%4)+1}`}><div/></div><h3>{p.name}</h3><p>{formatPrice(p.price)}</p></article>)}</div></section><section className="sf-story"><small>OUR APPROACH</small><h2>{editor.story.title}</h2><p>{editor.story.body}</p></section><section className="sf-newsletter"><h2>Notes from the studio.</h2><p>Occasional stories, new arrivals, and thoughtful things worth sharing.</p><div><span>Email address</span><button>Join</button></div></section><footer><strong>{data.businessName}</strong><span>Shop · About · Contact · Instagram</span><small>© 2026 {data.businessName}</small></footer></div>
}

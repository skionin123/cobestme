...
function OperationsPage({type,orders=[],customers=[],onCreate}) {
  const [adding,setAdding]=useState(false)
  const [busy,setBusy]=useState(false)
  const [error,setError]=useState('')
  ...
  if(type==='media') return <div className="page-wrap"><div className="page-head"><div><p className="overline">CONTENT</p><h1>Media</h1><p>Keep website and product assets organized in one library.</p></div><Button><Upload size={16}/> Add media</Button></div><EmptyState icon={ImageIcon} title="Media storage is the next production connector" body="The database model is ready for media records. File upload storage still needs to be connected before this button can upload real files."/></div>
  if(type==='analytics') {
  ...
  if(type==='marketing') return <div className="page-wrap"><div className="page-head"><div><p className="overline">GROWTH</p><h1>Marketing</h1><p>Organize store campaigns and customer touchpoints around the same business.</p></div><Button><Plus size={16}/> Create campaign</Button></div><EmptyState icon={Sparkles} title="Campaign storage is ready" body="The production database includes campaigns. Email delivery and campaign sending still require a transactional email provider." action="Create campaign"/></div>
  if(type==='discounts') return <div className="page-wrap"><div className="page-head"><div><p className="overline">COMMERCE</p><h1>Discounts</h1><p>Create offers that can be used across the storefront.</p></div><Button><Plus size={16}/> Create discount</Button></div><EmptyState icon={Sparkles} title="Discount storage is ready" body="The production database includes discount records. Checkout enforcement will be activated with the payment/checkout flow." action="Create discount"/></div>
  return null
}
